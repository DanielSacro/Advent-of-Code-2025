...
def main():
    ...
    for d in sorted(distances):
        ...
        for pair in distances[d]:
            ...
            if currPairs == maxPairs:
                break
        if currPairs == maxPairs:
            break

    # Find top 3 largest circuits out of the c circuits -> O(c)
    maxes = [0, 0, 0]
    for c in circuits:
        if len(circuits[c]) > maxes[0]:
            maxes[2] = maxes[1]
            maxes[1] = maxes[0]
            maxes[0] = len(circuits[c])
        elif len(circuits[c]) > maxes[1]:
            maxes[2] = maxes[1]
            maxes[1] = len(circuits[c])
        elif len(circuits[c]) > maxes[2]:
            maxes[2] = len(circuits[c])
